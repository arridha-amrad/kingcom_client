...
import { Field, Label, RadioGroup, Radio, Select, Textarea } from '@headlessui/react';
import { ChevronDownIcon } from 'lucide-react';
import type { CalculateShippingCostResponse, ShippingResponse } from './api/shipping.api';
...
type TShippingFormContext = {
  ...
  setAvailableCouriers: Dispatch<SetStateAction<CalculateShippingCostResponse[]>>;
  selectedCourier: CalculateShippingCostResponse | null;
  setSelectedCourier: Dispatch<SetStateAction<CalculateShippingCostResponse | null>>;
};
...
export default function ShippingForm({ children, carts }: { children: ReactNode; carts: Cart[] }) {
  const [targetProvince, setTargetProvince] = useState<null | ShippingResponse>(null);
  const [targetCity, setTargetCity] = useState<null | ShippingResponse>(null);
  const [targetDistrict, setTargetDistrict] = useState<null | ShippingResponse>(null);
  const [targetAddress, setTargetAddress] = useState('');
  const [availableCouriers, setAvailableCouriers] = useState<CalculateShippingCostResponse[]>([]);
  const [selectedCourier, setSelectedCourier] = useState<CalculateShippingCostResponse | null>(null);
...
ShippingForm.Destination = ({ children }: { children: ReactNode }) => {
  ...
  return <div className={cn('w-full space-y-4', availableCouriers.length > 0 ? 'hidden' : 'block')}>{children}</div>;
};
...
ShippingForm.SelectProvince = () => {
  ...
  return (
    <ShippingSelect isPending={isPending} label="Province" options={data?.provinces ?? []} setter={setTargetProvince} />
  );
};
...
ShippingForm.SelectCity = () => {
  ...
  return <ShippingSelect isPending={isPending} label="City" options={data?.cities ?? []} setter={setTargetCity} />;
};
...
ShippingForm.SelectDistrict = () => {
  ...
  return (
    <ShippingSelect isPending={isPending} label="District" options={data?.districts ?? []} setter={setTargetDistrict} />
  );
};
...
ShippingForm.InputHomeAddress = () => {
  ...
  return (
    <Field>
      <Label className="text-sm/6 font-medium text-foreground">Home Address</Label>
      <Textarea
        value={targetAddress}
...
ShippingForm.FindShippingServiceButton = () => {
  const { targetDistrict, carts, setAvailableCouriers } = useShippingFormContext();
...
ShippingForm.AvailableCouriers = () => {
  const { availableCouriers, selectedCourier, setSelectedCourier } = useShippingFormContext();
  return (
    <RadioGroup value={selectedCourier} onChange={setSelectedCourier} aria-label="Courier" className="space-y-2">
...
ShippingForm.PickCourier = ({ callback }: { callback?: VoidFunction }) => {
  const { selectedCourier, availableCouriers, targetAddress } = useShippingFormContext();
  ...
  return (
    <div onClick={chooseCourier} className={cn('my-4', availableCouriers.length > 0 ? 'block' : 'hidden')}>
...
const ShippingSelect = ({ isPending, label, options, setter }: ShippingSelectProps) => {
      ...
      <Field disabled={isPending}>
        ...
        <div className="relative">
          ...
          >
            ...
            {options?.map((o, i) => (
              <option className="text-white" key={i} value={`${o.id}_${o.name}`}>
                {o.name}
              </option>
            ))}
          </Select>
