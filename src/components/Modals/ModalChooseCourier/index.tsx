...
export default function ModalChooseCourier({}) {
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onClose={() => {}} className="z-50relative">
        <DialogBackdrop
          transition
        ...
        <div className="flex w-screen p-4 fixed inset-0 items-center justify-center">
          <DialogPanel
            transition
            className="z-50 max-w-sm w-full px-8 py-12 bg-background border border-foreground/20 rounded-2xl shadow-2xl relative backdrop-blur-2xl duration-300 ease-out data-closed:scale-95 data-closed:opacity-0"
          >
            <div className="bg-foreground/10 absolute inset-0 blur-3xl -z-50" />
            <button
              onClick={closeModal}
              className="absolute top-[4%] right-[5%]"
            >
              <X className="transition-colors stroke-foreground/50 hover:stroke-foreground ease-in duration-100" />
            </button>
