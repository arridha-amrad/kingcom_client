...
const ShippingForm = () => {
  const { data } = useQuery(provincesQueryOptions);
  const [targetProvince, setTargetProvince] = useState<null | ShippingResponse>(
    null,
  );

  const { data } = useQuery(citiesQueryOptions(targetProvince?.id));
  const [targetCity, setTargetCity] = useState<null | ShippingResponse>(null);

  const {
    data: { districts },
  } = useSuspenseQuery(districtsQueryOptions(targetCity?.id));
  const [district, setDistrict] = useState<null | ShippingResponse>(null);
  ...
  return (
    <div className="w-full space-y-2">
      <CheckoutSelect
        setData={setTargetProvince}
        label="Province"
        options={provinces}
      />
      <CheckoutSelect
        setData={setTargetCity}
        label="City"
        options={data?.cities ?? []}
      />
      <CheckoutSelect
